...
import { baseUrl, putProduct } from "../../../constants/urls";
import FormGeneralData from "../FormGeneralData";
import Politics from "../Politics";
import FormAtributes from "../FormAttributes";
...
const UpdateTemplate = ({ administrationForm, setAdministrationForm, productId }) => {
  ...
  const modifyProduct = async () => {
    try {
      await axios.put(baseUrl + putProduct + productId, data, { headers: headers });
      loaderSuccessful();
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className="formTemplateContainer">
      <h3 className="formTemplate__h3">Modificar propiedad</h3>
      <FormGeneralData
      ...
      <ReservationConfirmButton
        ...
        titleButton="Modificar hospedaje"
      />
    </div>
  );
};
